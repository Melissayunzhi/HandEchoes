...
async function setup() {
    ...
    instructionsDiv.html(
        ...
        "<ul>" +
        "<li>Draw or write with your right hand.</li>" +
...
function draw() {
    ...
    if (poses.length > 0) {
        ...
        let wrist = pose.keypoints.find(keypoint => keypoint.name === 'right_wrist');
        if (wrist) {
            wristX = wrist.x;
            wristY = wrist.y;
            ...
            if (isDrawing && !videoPaused) {
                ...
                if (i >= 0 && i < gridSize.x && j >= 0 && j < gridSize.y) {
                    grid[i][j] = 1;
                    colorGrid[i][j] = generateColor(wristX, wristY, i * CELL_SIZE, j * CELL_SIZE); // Store color when drawing
...
function generateColor(wristX, wristY, x, y) {
    let distanceToNose = dist(wristX, wristY, x, y);
...
function displayGrid(wristX, wristY) {
    if (showGrid) {
        stroke(255, 100);
        ...
        for (let j = 0; j < gridSize.y; j++) {
            ...
            if (grid[i][j] === 1) {
                if (videoPaused) {
                    ...
                    let color = generateColor(wristX, wristY, x, y);
